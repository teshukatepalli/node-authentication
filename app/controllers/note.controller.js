...
exports.create = (req,res) => {
    ...
    //Create a Note
    const note = new Note({
        title: req.body.title || "Undefined Note.",
...
exports.findAll = (req,res) => {
    Note.find().then(notes => {
        ...
        res.status(500).send({
            message: err.message || "Some error occurred while retrievinng notes."
        });
    });
};
...
exports.delete = (req,res) => {
    Note.findByIdAndRemove(req.params.noteId).then(note => {
        ...
        return send({message: "Note deleted succesfully"});
    }).catch(err => {
        if(err.kind === 'ObjectId' || err.name === 'NotFound') {
            return res.status(404).send({message: "Note not found"+ req.params.noteId})
        }
